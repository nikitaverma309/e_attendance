...
class FaceDetectorService {
  ...
  void initialize() {
    ...
    _faceDetector = FaceDetector(
      options: FaceDetectorOptions(
        ...
        performanceMode: FaceDetectorMode.accurate, // Performance mode: accurate for better quality
      ),
    );
    Utils.printLog('Face Detector Initialized');
  }
  ...
  Future<void> detectFacesFromImage(CameraImage image) async {
    try {
      ...
      final metadata = InputImageMetadata(
        ...
        bytesPerRow: image.planes[0].bytesPerRow,  // Typically for NV21 or YUV420 format
      );
  ...
  Future<imglib.Image?> cropFaceFromImage(CameraImage cameraImage) async {
    try {
      ...
      final metadata = InputImageMetadata(
        size: Size(cameraImage.width.toDouble(), cameraImage.height.toDouble()),
        rotation: _cameraService.cameraRotation ?? InputImageRotation.rotation0deg,
        format: InputImageFormatValue.fromRawValue(cameraImage.format.raw) ?? InputImageFormat.yuv_420_888,
        bytesPerRow: cameraImage.planes[0].bytesPerRow,  // Adjust as per your image's first plane
      );
